...
return(<>
    ...
    { value !== null && !Array.isArray(value) || Array.isArray(value) && value.length !== 0  ? Array.isArray(value) && value.length > 0 ? <div className="flex gap-6 bg-[#cadbd0] flex-wrap">{ value.map((file, index) => <div className="relative"><img key={index} className="w-30 h-30 object-cover self-center" src={file instanceof Blob ? URL.createObjectURL(file) : file}/><button type="button" onClick={() => { removeImages(index) }} className="absolute top-0 right-1 cursor-pointer"><Icon icon="material-symbols:cancel-outline-rounded" width="30px" height="30px"  style={{color: '#910a00'}} /></button></div>) } </div> : <div className="w-60 self-center relative"> <img className="w-60 object-cover self-center" src={value instanceof Blob ? URL.createObjectURL(value) : value} /><button onClick={removeFile} className="absolute top-0 right-1 cursor-pointer"><Icon icon="material-symbols:cancel-outline-rounded" width="30px" height="30px"  style={{color: '#910a00'}} /></button></div> :
    <>
